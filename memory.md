...
- **2026-07-19**: Fixed Khatabook-style XLSX import header detection — `getAllRecords()` rewritten to skip leading title rows and detect actual header row. Also fixed auto-mapping with fuzzy matching (includes-based). See `import.service.ts`.
- **2026-07-19**: Added Khatabook transaction import to customer import — `importCustomers()` now detects debit/credit columns and creates ledger entries with running balance. Groups transactions by customer name, creates customer if not exists. See `import.service.ts`.
- **2026-07-19**: Fixed `getNormalized()` returning undefined for class-transformer properties — class-transformer creates DTO instances with declared properties set to undefined; `name` key matched before `Name`. Fixed by skipping null/empty values in normalization passes. See `import.service.ts`.
...
- **2026-07-19**: Fixed Android bugs — API sent `per_page` but backend expects `limit` (all pagination capped at 20); DashboardData model fields mismatched backend (totalSales/totalBilled/pendingAmount/outstanding); GSTR routes wrong (`gstr1` vs `gstr-1`); Invoice.customerId null causes NPE in toEntity() mapping; ProductListViewModel.init blocking collect prevents refresh. See commit c3af55d.
- **2026-07-19**: Fixed more Android crashes — Gstr1Report.b2c non-nullable but backend returns null (NPE at ReportsScreen.kt:660); CustomerReport field names mismatched backend (`id`/`name`/`balance`/`totalInvoices` vs `customerId`/`customerName`/`outstanding`/`totalPurchases`); made all GSTR models nullable-safe with defaults; GSTR query params fixed (`from_date`→`fromDate`, `gstr1`→`gstr-1`).
- **2026-07-20**: Major Android overhaul — Fixed all API endpoint URLs (`businesses/{id}/` prefix for customers/products/suppliers/purchase-orders/ledger); Added `@SerializedName` annotations to Invoice/Payment/InvoiceItem for backend field alignment; Added `remoteId` to CustomerEntity/InvoiceEntity fixing navigation double-hash bug; Added InvoiceDetailScreen actions (Edit, Delete, Share, PDF); Added CreateInvoiceScreen edit support; Added BusinessListScreen (switch/create business + staff invite accept); Added supplier edit/delete, PO cancel; Added product edit loadProduct; Added business switcher to dashboard/settings; DB version 6 (destructive migration). APK: 43.5 MB.
- **2026-07-22**: taxType selector (CGST+SGST/IGST) across web, Android, iOS; fixed Amount circular dependency (treats as final total incl. tax); fixed Android tax double-count bug; fixed Android Amount field backspace decimal forcing; added email-based register/login (phone+email both collected, login with either); added OTP login tab to admin dashboard; added staff invite accept page + login redirect flow; added change password (old→new) + forgot/reset password via OTP across backend, web, Android; fixed deprecated ArrowBack/menuAnchor APIs on Android.
