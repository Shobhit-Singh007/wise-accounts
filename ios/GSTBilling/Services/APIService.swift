...
struct ImportRecordsRequest: Encodable {
...
final class APIService {
    ...
    func lookupGstin(businessId: String, gstin: String) async throws -> GSTINLookupResult {
        try await request(.customers(businessId), endpoint: "gstin/\(gstin)")
    }
    ...
    // Import
    func importCustomers(businessId: String, records: [[String: Any]]) async throws -> ImportResponse {
        let url = "\(Constants.baseURL)/businesses/\(businessId)/import/customers"
        let body = ImportRecordsRequest(records: records.map { dict in dict.mapValues { AnyCodable($0) } })
        return try await requestRaw(path: url, method: "POST", body: body)
    }

    func importProducts(businessId: String, records: [[String: Any]]) async throws -> ImportResponse {
        let url = "\(Constants.baseURL)/businesses/\(businessId)/import/products"
        let body = ImportRecordsRequest(records: records.map { dict in dict.mapValues { AnyCodable($0) } })
        return try await requestRaw(path: url, method: "POST", body: body)
    }

    func importInvoices(businessId: String, records: [[String: Any]]) async throws -> ImportResponse {
        let url = "\(Constants.baseURL)/businesses/\(businessId)/import/invoices"
        let body = ImportRecordsRequest(records: records.map { dict in dict.mapValues { AnyCodable($0) } })
        return try await requestRaw(path: url, method: "POST", body: body)
    }
